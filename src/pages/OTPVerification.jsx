...
const OTPVerification = () => {
  ...
  useEffect(() => {
    // Redirect if no email in session storage
    if (!email) {
      navigate("/reset-password");
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (result.status === 200) {
        ...
        setError(result.response?.data?.message || "Invalid OTP. Please try again.");
      }
    } catch (err) {
  ...
  const handleResendOTP = async () => {
    ...
    try {
      ...
      if (result.status === 200) {
        ...
        setError(result.response?.data?.message || "Failed to resend OTP. Please try again.");
      }
    } catch (err) {

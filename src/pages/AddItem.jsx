...
//                          showAlert(`Item added successfully!: ${result?.data?.message || 'Unknown error'}`, 'sucess');
...
//                                                                     onClick={() => handleStageTypeSelect(stage)}
//                                                                 >
//                                                                     {stage}
//                                                                 </div>
//                                                             ))
        ...
        if (isValid) {
            ...
            if (token) {
                ...
                try {
                    ...
                    if (result && result.status === 200) {
                        // Fixed this line - only show success message with success type
                        ...
                        // Add a small delay before navigation to allow the alert to be seen
                        setTimeout(() => {
                            if (formData.festival) {
                                navigate(`/ItemRegistrationList?festival=${encodeURIComponent(formData.festival)}`);
                            } else {
                        ...
                        showAlert(` ${result?.data?.message || 'Unknown error'}`, 'error');
                    }
                } catch (err) {
                    console.error("Error adding item:", err);
                    showAlert("Error adding item. Please try again.", 'error');

...
const DItemCodewise = () => {
  ...
  const getPrintTitle = () => {
    ...
    // Get the festival part of the title
    switch (selectedFestival) {
      case "UP Kalaivizha":
      ...
      case "HSS Kalaivizha":
        ...
        break;
      case "All Festival":
        festivalTitle = "All Festival Tamil Kalaivizha";
        break;
      default:
        festivalTitle = "Tamil Kalaivizha";
    }
    
    // Include the item name in the title if available
    if (itemNameDisplay) {
      return `${festivalTitle} - ${itemNameDisplay} (${schoolCodeSearch}) - Point Report`;
    } else {
      // If no specific item name is available, use the original title format
      return `${festivalTitle} - Item Code Wise Point Report`;
    }
  };
  const handlePrint = () => {
    // Create a clean container for the PDF content
    ...
    // Generate a better filename that includes the item code if available
    let fileName = '';
    if (schoolCodeSearch) {
      fileName = `${selectedFestival.replace(/ /g, '_')}_${schoolCodeSearch}_Points.pdf`;
    } else {
      fileName = `${selectedFestival.replace(/ /g, '_')}_Item_Wise_Points.pdf`;
    }
